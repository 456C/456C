...
execute if score .leverTimer obj matches 1 as @e[type=marker, tag = Lever] at @s run playsound block.iron_door.open master @a[distance=..15] ~ ~ ~ 100 0.1
execute if score .leverTimer obj matches 1 run fill 518 3 494 519 3 493 air
execute if score .leverTimer obj matches 1 as @e[type=marker, tag = Lever] at @s run particle campfire_cosy_smoke ~ ~ ~ 0 0 0 0 2 
